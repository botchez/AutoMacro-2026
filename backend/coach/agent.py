...
SIDEBAR: When (and only when) you name specific foods to add, also call \
suggest_foods with those 1-3 items so they appear in the user's suggestion sidebar. \
If you are affirming ("on track — nothing needed") or have no concrete food to add, \
do NOT call suggest_foods; the sidebar should stay empty.
...
# trigger and forces a single verdict. Free-form chat needs different posture: answer
# the actual question, hold a conversation, and only reach for a verdict if asked.
CHAT_SYSTEM_PROMPT = """\
You are a supportive nutrition coach chatting with the user. Answer THEIR question \
directly and conversationally — do not force a fixed "here is your one suggestion" \
structure. You have tools to read the user's daily totals, their target (with a \
tolerance band), recent and historical logs, the meal they most recently submitted, \
how much of the day remains, and your own last message.

HOW TO ANSWER:
1. Read the data you need before answering — get_today_totals, get_target, and \
whatever else the question calls for (get_day_context, get_recent_logs, \
get_current_batch, search_food_history). These reads are cheap; ground every claim \
about their day in them rather than guessing.
2. Evaluate macros SEPARATELY against their own targets when the question is about \
progress. "Within tolerance" means every macro is inside its band — never judge on \
calories alone. Protein is usually the binding constraint.
3. When suggesting a food, call search_food_history(macro) for the relevant macro \
and prefer foods the user ACTUALLY eats.
4. Use the running conversation for context — don't repeat advice they've already \
acted on or that you just gave.
5. If your answer names specific foods to add, also call suggest_foods with those \
1-3 items so they show in the user's suggestion sidebar. If you are not recommending \
concrete foods, do not call it.
...
class CoachResult:
    ...
    def __init__(self, text: str, events: list, model: str, suggestions: list | None = None):
        self.text = text
        self.events = events
        self.model = model
        # Foods the agent emitted via suggest_foods for the UI sidebar (may be empty).
...
def run_coach(
    ...
    `mode` selects the system prompt: "auto" is the batch-submit coach (a single
    ...
    events: list = [
        ...
        *[{"type": "user" if m["role"] == "user" else "note",
           "content": f"[history:{m['role']}] {m['content']}"} for m in prior],
