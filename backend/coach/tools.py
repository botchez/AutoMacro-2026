...
def suggest_foods(state: CoachState, items: list) -> dict:
    """Write tool: register the foods to show in the user's suggestion sidebar.

    Unlike the reads above this mutates state — it replaces whatever the agent
    suggested earlier in the same run, so the model can call it once with its final
    picks. The service reads state.suggestions back out after the loop.
    """
...
def _validate(name: str, args: dict) -> dict:
    ...
    if name == "suggest_foods":
        raw = args.get("items")
        if not isinstance(raw, list):
            raise ValueError("items must be a list of {name, serving, reason}")
        items: list[dict] = []
        for entry in raw[:3]:  # sidebar shows a few; ignore the rest
            if not isinstance(entry, dict):
                continue
            food = str(entry.get("name", "")).strip()[:80]
            if not food:
                continue
            items.append({
                "name": food,
                "serving": (str(entry.get("serving", "")).strip()[:40] or "1 serving"),
                "reason": str(entry.get("reason", "")).strip()[:200],
            })
        if not items:
            raise ValueError("items had no usable {name, serving, reason} entries")
        return {"items": items}
...
TOOL_SCHEMAS = [
    ...
    _tool("suggest_foods",
          "Fill the user's on-screen suggestion sidebar with specific foods to add. "
          "Call this ONLY when you are actually recommending foods (e.g. to close a "
          "macro gap). Skip it entirely when the user is on track or you have no "
          "concrete food to add — the sidebar then stays empty. Prefer foods from "
          "search_food_history. Give 1-3 items.",
          {"items": {
              "type": "array",
              "description": "Foods to show, most useful first.",
              "items": {
                  "type": "object",
                  "properties": {
                      "name": {"type": "string",
                               "description": "Food name, e.g. 'Greek yogurt'."},
                      "serving": {"type": "string",
                                  "description": "Amount to add, e.g. '100 g', '1 medium'."},
                      "reason": {"type": "string",
                                 "description": "One short phrase on why it helps."},
                  },
                  "required": ["name", "serving", "reason"],
              },
          }},
          ["items"]),
]
